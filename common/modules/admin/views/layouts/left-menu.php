...
$routeWithoutAction = str_replace('/'.Yii::$app->controller->action->id,'',$route);
foreach ($menus as $i => $menu) {
//    $menus[$i]['active'] = strpos($route, trim($menu['url'][0], '/')) === 0;
    $menus[$i]['active'] = $routeWithoutAction == trim($menu['url'][0], '/');
}
...
<?php
list(, $url) = Yii::$app->assetManager->publish('@common/modules/admin/assets');
